package com.example.innerpeace_database;
    ...
    private class NotificationManagerBuilder extends AsyncTask<Void, Void, Notification> {
        ...
        private void updateRemoteView(RemoteViews remoteViews, Notification notification) {
            if (mService.isPlaying()) {
                remoteViews.setImageViewResource(R.id.btn_play_pause, R.drawable.ic_baseline_music_note_24);
            } else {
                remoteViews.setImageViewResource(R.id.btn_play_pause, R.drawable.ic_baseline_music_note_24);
            }
            ...
            Picasso.get().load(albumArtUri).error(R.drawable.ic_baseline_music_note_24).into(remoteViews, R.id.img_albumart, NOTIFICATION_PLAYER_ID, notification);
        }
